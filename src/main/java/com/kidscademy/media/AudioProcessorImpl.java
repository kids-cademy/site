...
public class AudioProcessorImpl implements AudioProcessor
{
  ...
  public void generateWaveform(File audioFile, File waveformFile) throws IOException
  {
    ...
    String filter = format("showwavespic=size=${width}x${height}:colors=${color}:scale=lin", WAVEFORM_WIDTH, Math.round(H), WAVEFORM_COLOR);
  ...
  private static <T extends ResultParser> T exec(Class<T> resultClass, String format, Object... args) throws IOException
  {
  ...
  private static Process start(List<String> command) throws IOException
  {
    ...
    // redirect STDERR to STDOUT so that reading process.getInputStream get them both
