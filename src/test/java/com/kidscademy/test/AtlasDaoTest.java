...
public class AtlasDaoTest
{
  ...
  private static void assertInstrument(Instrument instrument) throws MalformedURLException
  {
    ...
    assertThat(instrument.getRepositoryIndex(), equalTo(1));
    ...
    assertThat(instrument.getRelated().get(0), equalTo(2));
    assertThat(instrument.getRelated().get(1), equalTo(3));
  }
  ...
  public void saveInstrument() throws MalformedURLException
  {
    ...
    instrument.setRepositoryIndex(2);
    ...
    spreading.add(new Region("Europe", Region.Area.S));
    spreading.add(new Region("Africa", Region.Area.N));
  ...
  public void saveInstrument_XP() throws MalformedURLException
  {
    ...
    Link link = new Link(new URL("https://en.wikipedia.org/wiki/Accordion"), "Wikipedia-1", "Accordion article on wikipedia.", "wikipedia.png");
    dao.saveLink(link);
  ...
  {
    Link link = new Link(new URL("https://en.wikipedia.org/wiki/Piano"), "Wikipedia", "Piano article on wikipedia.", "wikipedia.png");
    dao.saveLink(link);
    assertThat(link.getId(), not(equalTo(0)));
  }
}
