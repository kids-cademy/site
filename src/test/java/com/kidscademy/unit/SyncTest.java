...
public class SyncTest extends TestCase
{
  ...
  /**
   * Target directory is updated with files not existing on source directory. Dirty files should contain stale files but
   * no fresh ones.
   * <p>
   * In this test case source files are [1, 7, 8] and target files [1, 4, 5, 6, 7, 8]. Sources and related targets
   * content is the same. After test exercise dirty files stale files contain [4, 5, 6] and fresh files list is empty
  ...
  /**
   * Target directory is updated with files not existing on source directory and source directory with files not
   * existing on target. Dirty files should contain both fresh and stale files.
   * <p>
   * In this test case source files are [1, 2, 3, 7, 8, 9] and target files [0, 1, 4, 6, 7, 8]. Content of the files
   * present on both source and target is the same, i.e. files [1, 7, 8] are identical. After test exercise, dirty files
