...
public class OrderColumnTest
{
  ...
  /**
   * Load children list directly with JPQL, that is, not as part of its parent. Although generated SQL retrieves children_order, returned list is ordered by id
   * column. This is in contrast with loading children list via parent, where list is ordered by children_order column.
   * <p>
   * Result is somehow surprising: why JPA implementation retrieves children_order column if ignore it? Since children_order is part of generated SQL it is
   * clear that implementation knows about parent {@literal}OrderColum annotation, but actively choose not to use it.
   * <p>
   * APIDOC does not state something about this condition. Answer on SO consider this condition as normal.
   */
  @Test
  ...
  public void mergeReorderedList() {
    ...
    String jpql = "select p from Parent p where p.id=:id";
    em = factory.createEntityManager();
    Parent persistedParent = em.createQuery(jpql, Parent.class).setParameter("id", parent.getId()).getSingleResult();
    em.close();
